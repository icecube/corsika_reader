...
namespace corsika
{
    ...
    /**
     ...
     \brief Defines units in terms of CORSIKA standard units
     ...
     convert your data into CORSIKA base units, so that
     ...
     This is a slightly modified version of the units definitions written by the Pierre Auger collaboration,
     which in turn is a slightly modified version of the units definitions written by the Geant4 collaboration
    ...
    // Length [L]
    //
    const double meter  = 100.0; // unit is cm
    ...
    // Time [T]
    //
    const double nanosecond  = 1; // unit is nanosecond
    ...
    // Energy [E]
    //
    const double electronvolt      = 1e-9; // unit is GeV
